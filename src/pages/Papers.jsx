...
const TAG_MAP = {
  ...
  "Analise-Agrupamento": { label: "#Analise-Agrupamento", color: "bg-purple-100 text-purple-900 border-purple-300 dark:bg-purple-100 dark:text-purple-950 dark:border-purple-300" },
  "Analise-Incerteza": { label: "#Analise-Incerteza", color: "bg-violet-100 text-violet-900 border-violet-300 dark:bg-violet-100 dark:text-violet-950 dark:border-violet-300" },
  "Analise-Sensibilidade": { label: "#Analise-Sensibilidade", color: "bg-indigo-100 text-indigo-900 border-indigo-300 dark:bg-indigo-100 dark:text-indigo-950 dark:border-indigo-300" },
  "Aproveitamento-agua-chuva": { label: "#Aproveitamento-agua-chuva", color: "bg-sky-100 text-sky-900 border-sky-300 dark:bg-sky-100 dark:text-sky-950 dark:border-sky-300" },
  "Certificacao-Ambiental": { label: "#Certificacao-Ambiental", color: "bg-emerald-100 text-emerald-900 border-emerald-300 dark:bg-emerald-100 dark:text-emerald-950 dark:border-emerald-300" },
  "Desempenho-Termico": { label: "#Desempenho-Termico", color: "bg-amber-100 text-amber-900 border-amber-300 dark:bg-amber-100 dark:text-amber-950 dark:border-amber-300" },
  "Eficiencia-Energetica": { label: "#Eficiencia-Energetica", color: "bg-yellow-200 text-yellow-950 border-yellow-400 dark:bg-yellow-200 dark:text-yellow-950 dark:border-yellow-400" },
  "Emissoes-Carbono": { label: "#Emissoes-Carbono", color: "bg-rose-100 text-rose-900 border-rose-300 dark:bg-rose-100 dark:text-rose-950 dark:border-rose-300" },
  "Energia-Solar": { label: "#Energia-Solar", color: "bg-amber-100 text-amber-900 border-amber-300 dark:bg-amber-100 dark:text-amber-950 dark:border-amber-300" },
  "Energia-embutida": { label: "#Energia-embutida", color: "bg-orange-100 text-orange-900 border-orange-300 dark:bg-orange-100 dark:text-orange-950 dark:border-orange-300" },
  "First-Flush": { label: "#First-Flush", color: "bg-cyan-100 text-cyan-900 border-cyan-300 dark:bg-cyan-100 dark:text-cyan-950 dark:border-cyan-300" },
  "Hidraulica": { label: "#Hidraulica", color: "bg-blue-100 text-blue-900 border-blue-300 dark:bg-blue-100 dark:text-blue-950 dark:border-blue-300" },
  "Pneu": { label: "#Pneu", color: "bg-stone-200 text-stone-900 border-stone-400 dark:bg-stone-200 dark:text-stone-900 dark:border-stone-400" },
  "Precipitacao": { label: "#Precipitacao", color: "bg-sky-100 text-sky-900 border-sky-300 dark:bg-sky-100 dark:text-sky-950 dark:border-sky-300" },
  "Preventivo-Incendio": { label: "#Preventivo-Incendio", color: "bg-red-100 text-red-900 border-red-300 dark:bg-red-100 dark:text-red-950 dark:border-red-300" },
  "Qualidade-Agua": { label: "#Qualidade-Agua", color: "bg-teal-100 text-teal-900 border-teal-300 dark:bg-teal-100 dark:text-teal-950 dark:border-teal-300" },
  "Radiacao-solar": { label: "#Radiacao-solar", color: "bg-amber-100 text-amber-900 border-amber-300 dark:bg-amber-100 dark:text-amber-950 dark:border-amber-300" },
  "Reservatorio": { label: "#Reservatorio", color: "bg-blue-100 text-blue-900 border-blue-300 dark:bg-blue-100 dark:text-blue-950 dark:border-blue-300" },
  "Resíduo": { label: "#Resíduo", color: "bg-stone-200 text-stone-900 border-stone-400 dark:bg-stone-200 dark:text-stone-900 dark:border-stone-400" },
  "Retrofit": { label: "#Retrofit", color: "bg-teal-100 text-teal-900 border-teal-300 dark:bg-teal-100 dark:text-teal-950 dark:border-teal-300" },
  "Transporte": { label: "#Transporte", color: "bg-slate-200 text-slate-900 border-slate-400 dark:bg-slate-200 dark:text-slate-900 dark:border-slate-400" },
  "Ventilacao": { label: "#Ventilacao", color: "bg-cyan-100 text-cyan-900 border-cyan-300 dark:bg-cyan-100 dark:text-cyan-950 dark:border-cyan-300" },
  "Vidros-E-Janelas": { label: "#Vidros-E-Janelas", color: "bg-sky-100 text-sky-900 border-sky-300 dark:bg-sky-100 dark:text-sky-950 dark:border-sky-300" },
};

function TagBadge({ tagId, selectedTag, onSelectTag }) {
  const { t } = useTranslation();
  const meta = TAG_MAP[tagId];
  const isSelected = selectedTag === tagId;
  const label = t(`tags.${tagId}`, tagId);
  const colorClass = meta ? meta.color : "bg-slate-100 text-slate-800 border-slate-300 dark:border-zinc-700";
  ...
  return (
    <button
      ...
      onClick={(e) => {
        e.stopPropagation();
        if (onSelectTag) {
          onSelectTag(isSelected ? "all" : tagId);
        }
      }}
      className={
        "inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold border transition-all cursor-pointer " +
        colorClass +
        (isSelected ? " ring-2 ring-emerald-500 scale-105 shadow-sm" : " hover:opacity-80")
      }
    >
      #{label}
    </button>
...
function TagLollipopChart({ papers, selectedTag, onSelectTag, searchQuery }) {
  const [hoveredTag, setHoveredTag] = useState(null);
  const { t } = useTranslation();

  // Calculate tag counts across the passed papers (which reflect search query)
  const tagCountsMap = {};
  Object.values(papers).forEach(paperList => {
  ...
  // Sort tags descending by count
  const sortedTagList = Object.entries(tagCountsMap)
    .map(([id, count]) => ({
      id,
      count,
      name: t(`tags.${id}`, id),
  ...
  const maxCount = sortedTagList.length > 0 ? sortedTagList[0].count : 1;
  const isSearchActive = searchQuery && searchQuery.trim() !== "";

  return (
    <div className="bg-white dark:bg-slate-50 border border-emerald-200 dark:border-slate-300 rounded-2xl p-4 shadow-md relative flex flex-col justify-between">
      <div>
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-sm font-extrabold text-slate-900 dark:text-slate-900 flex items-center gap-2">
            <span>{t("papers.chart_lollipop_title", "Frequência de Temas (Tags)")}</span>
            {isSearchActive && (
              <span className="text-[10px] text-emerald-900 font-bold bg-emerald-100 px-2 py-0.5 rounded-full border border-emerald-300">
                {t("papers.search_filter_active", "Filtrado por busca")}
              </span>
            )}
          </h3>
          {selectedTag !== "all" && (
            <button
              ...
              className="text-[11px] text-emerald-800 dark:text-emerald-900 font-bold bg-emerald-100 hover:bg-emerald-200 px-2 py-0.5 rounded-full border border-emerald-300 transition-colors cursor-pointer"
            >
              {t("papers.clear_filter", "Limpar filtro")} ✕
            </button>
          )}
        </div>
        <p className="text-[11px] text-slate-600 mb-3 font-bold">
          {isSearchActive
            ? t("papers.lollipop_sub_search", "Temas presentes nos artigos encontrados na busca")
            : selectedTag !== "all"
            ? t("papers.lollipop_sub_selected", "Clique novamente para desmarcar o tema")
            : t("papers.lollipop_sub", "Clique em um tema para destacar no gráfico anual")}
        </p>

        {sortedTagList.length === 0 ? (
          <div className="text-center py-8 text-xs text-slate-500 font-bold italic">
            {t("papers.no_tags_found", "Nenhum tema encontrado para a busca.")}
          </div>
        ) : (
          <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1 custom-scrollbar">
            {sortedTagList.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  onClick={() => onSelectTag(isSelected ? "all" : item.id)}
                  onMouseEnter={() => setHoveredTag(item.id)}
                  onMouseLeave={() => setHoveredTag(null)}
                  className={
                    "flex items-center gap-2 text-xs cursor-pointer p-1.5 rounded-xl transition-all duration-150 " +
                    (isSelected
                      ? "bg-emerald-100/90 dark:bg-emerald-100 ring-2 ring-emerald-500 font-black shadow-sm"
                      : isHovered
                      ? "bg-slate-100/80 dark:bg-slate-200/60 font-bold"
                      : "hover:bg-slate-50 font-semibold")
                  }
                >
                  {/* Tag Name Label */}
                  <div className="w-36 sm:w-44 shrink-0 truncate text-slate-900 dark:text-slate-900" title={item.name}>
                    {item.name}
                  </div>

                  {/* Lollipop Line + Circle Bar */}
                  <div className="flex-1 flex items-center relative h-5 pr-6">
                    {/* Stem line */}
                    <div className="w-full bg-slate-200 dark:bg-slate-300 h-1 rounded-full relative">
                      <div
                        className={
                    ...
                    <div
                      className={
                        "absolute transform -translate-x-1/2 flex items-center justify-center transition-all duration-300 shadow-sm " +
                        (isSelected
                          ? "w-6 h-6 bg-emerald-600 text-white rounded-full font-extrabold text-[10px] ring-2 ring-emerald-300"
                          : isHovered
                          ? "w-5 h-5 bg-emerald-500 text-white rounded-full font-bold text-[9px]"
                          : "w-4.5 h-4.5 bg-slate-600 dark:bg-slate-700 text-white rounded-full font-bold text-[9px]")
                      }
                      style={{ left: `calc(${pct}%)` }}
                    >
                      {item.count}
...
export default function Papers() {
  ...
  const allPapers = {
    "2026": [
        ...
        {
            "id": 9,
            "authors": "MATOS, I. R. ; VAZ, I. C. M. ; CUSTODIO, D. A. ; GHISI, E.",
            "title": "Compliance of fire prevention systems in educational institutions: a case study in Santa Catarina",
            "details": "AMBIENTE CONSTRUÍDO (ONLINE), v. 26, p. e152235, 2026.",
            "year": "2026",
            "citations": "",
        ...
        {
            "id": 10,
            "authors": "BEZERRA, I. W. ; CUSTODIO, D. A. ; VAZ, I. C. M. ; GHISI, E.",
            "title": "Economic viability of a rainwater harvesting system in a single-family home in southern Brazil",
            "details": "AMBIENTE CONSTRUÍDO (ONLINE), v. 26, p. e152200, 2026.",
            "year": "2026",
            "citations": "",
        ...
        {
            "id": 11,
            "authors": "VIANA, M. R. ; LOESER, B. C. ; CANDIDO, C. ; GHISI, E.",
            "title": "Occupants' adaptive behaviour and thermal perception in social housing buildings in Brazil",
            "details": "ENERGY AND BUILDINGS, v. 369, p. 117943, 2026.",
            "year": "2026",
            "citations": "",
    ...
    "2023": [
        {
            ...
            "year": "2023",
            "citations": "",
            "doi": "https://www.scielo.br/j/ac/a/Db7ntHnfBKwLqSrJWsyM9Tt/?lang=en",
            "tags": [
                "Agua-Pluvial",
                "Agua-Potavel",
                "Residencial-Multifamiliar",
  ...
  const allTagIds = Array.from(
    new Set(Object.values(allPapers).flatMap(list => list.flatMap(p => p.tags || [])))
  ).sort((a, b) => t(`tags.${a}`, a).localeCompare(t(`tags.${b}`, b)));
  ...
  // Compute search-filtered papers across all years (ignoring selectedTag, to show full tag distribution of search results)
  const searchFilteredPapers = {};
  Object.entries(allPapers).forEach(([year, list]) => {
    const matched = list.filter(p => {
      if (searchQuery.trim() === "") return true;
      const q = searchQuery.toLowerCase();
      const matchesTitle = p.title.toLowerCase().includes(q);
      const matchesAuthors = p.authors.toLowerCase().includes(q);
      const matchesDetails = p.details.toLowerCase().includes(q);
      const matchesTags = (p.tags || []).some(tag => {
        const tagName = t(`tags.${tag}`, tag);
        return tagName.toLowerCase().includes(q);
      });
      return matchesTitle || matchesAuthors || matchesDetails || matchesTags;
    });
    if (matched.length > 0) {
      searchFilteredPapers[year] = matched;
    }
  });
                            ...
                            {(paper.tags || []).length > 0 && (
                              <div className="flex flex-wrap gap-1.5 mt-2">
                                {(paper.tags || []).map(tagId => (
                                  <TagBadge key={tagId} tagId={tagId} selectedTag={selectedTag} onSelectTag={setSelectedTag} />
                                ))}
                              </div>
                            )}
                            {paper.doi && (
